...
interface IUseMutationResult {
  ok: boolean;
  product: Product; //Product type은 prisma에서 model을 만드는 순간 제공
}

const Upload: NextPage = () => {
  const router = useRouter();
  const [uploadProduct, { loading, data }] =
    useMutation<IUseMutationResult>("/api/products"); //해당 url로 fetch하는 uploadProduct func 반환
  const { register, handleSubmit, watch } = useForm<IUploadForm>();
  const onValid = async ({ name, description, price, photo }: IUploadForm) => {
    if (loading) return;
    if (photo && photo.length > 0) {
      ...
      uploadProduct({ name, description, price }); //api url로 data fetch
    }
  };
  useEffect(() => {
    if (data?.ok) {
