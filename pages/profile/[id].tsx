...
const othersProfile = () => {
  ...
  const { data } = useSWR<IOtherProfile>(`/api/users/${router.query.id}`);
  return (
    <Layout title={data?.profile.name} hasTabBar seoTitle={data?.profile.name}>
      {data?.profile ? (
        <div className="px-4">
          <div className="flex items-center space-x-3 mt-4">
            {data.profile.avatar ? (
              <img
                src={`https://imagedelivery.net/H_yIPSozL5v7ZLv9PjoVyA/${data.profile.avatar}/avatar`}
